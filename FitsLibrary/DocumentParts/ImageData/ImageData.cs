...
{
    public class ImageData<T> : DataContent where T : INumber<T>
    {
        private int[] AxisIndexFactors { get; set; }
        private int[] AxisSizes { get; set; }
        ...
        public ImageData(int[] axisSizes, Memory<T> rawData)
        {
        ...
        public T GetValueAt(params int[] coordinates)
        {
            ...
            var index = this.GetIndexByCoordinates(coordinates);
